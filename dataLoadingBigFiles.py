...
changeDTypes = {'NPI': 'Int64','Entity Type Code': 'Int64','Provider Organization Name (Legal Business Name)': 'string[pyarrow]','Provider Last Name (Legal Name)': 'string[pyarrow]',
...
'Provider Business Mailing Address Postal Code': 'Int64','Provider Business Mailing Address Country Code (If outside U.S.)': 'string[pyarrow]',
'Provider Business Mailing Address Telephone Number': 'Int64','Provider Business Mailing Address Fax Number': 'Int64','Provider First Line Business Practice Location Address': 'string[pyarrow]',
...
'Provider Business Practice Location Address State Name': 'string[pyarrow]','Provider Business Practice Location Address Postal Code': 'Int64',
'Provider Business Practice Location Address Country Code (If outside U.S.)': 'string[pyarrow]','Provider Business Practice Location Address Telephone Number': 'Int64',
...
'Authorized Official Title or Position': 'string[pyarrow]','Authorized Official Telephone Number': 'Int64','Healthcare Provider Taxonomy Code_1': 'string[pyarrow]','Provider License Number_1': 'string[pyarrow]','Provider License Number State Code_1': 'string[pyarrow]','Healthcare Provider Taxonomy Code_2': 'string[pyarrow]','Healthcare Provider Taxonomy Code_3': 'string[pyarrow]',
...
# df = pd.read_csv(fileName, dtype_backend='pyarrow', low_memory=True, usecols=theCols, nrows=rowsToProcess)
df = ddf.read_csv(fileName, usecols=theCols, assume_missing=True, dtype=changeDTypes)  
## use pyarrow to reduce datatype memory usage
# dtype={
#    'column1': 'Int64',
#    'column2': 'float32',
#    'column3': 'category',
#    # Add more columns as needed
print(f"Step 1: Loaded variable 'df' from URI: {fileName}")
df.dtypes
...
df.compute()
print(f"Step 3: Renamed column 'Is Sole Proprietor' to 'Sole Proprietor'")	
...
df['Address Type'] = df['Provider First Line Business Mailing Address'].apply(
    lambda x: 'BM' if pd.notnull(x) and x.strip() != '' else 'UNK', meta=('Address Type', 'string[pyarrow]')
)
df.compute()
print(f"Step 4: Added 'Address Type' column based on 'Provider First Line Business Mailing Address'")
...
df.compute()
print(f"Step 5: Created new columns Address1, Address2, City, State, Zip, Country, Phone from mailing address fields")
...
df_diff = df[df["Provider First Line Business Mailing Address"] != df["Provider First Line Business Practice Location Address"]]

df_diff = df_diff.assign(
    Address_Type="BP",
    Address1=df_diff["Provider First Line Business Practice Location Address"],
    Address2=df_diff["Provider Second Line Business Practice Location Address"],
    City=df_diff["Provider Business Practice Location Address City Name"],
    State=df_diff["Provider Business Practice Location Address State Name"],
    Zip=df_diff["Provider Business Practice Location Address Postal Code"],
    Country=df_diff["Provider Business Practice Location Address Country Code (If outside U.S.)"],
    Phone=df_diff["Provider Business Practice Location Address Telephone Number"]
)

df = ddf.concat([df, df_diff])
df.compute()
print(f"Step 6: Duplicated rows where mailing and practice addresses differ, set 'Address Type' to 'BP', and updated Address1")
...
df.compute()
print(f"Step 7: Defaulted empty names to authorized names for 'Provider Last Name (Legal Name)' and 'Provider First Name'")
...
df.compute()
print(f"Step 8: Removed specified columns from DataFrame: {columns_to_remove}")	
...
df.compute()
print(f"Step 9: Renamed columns based on mapping from 'New Column Names' sheet")
...
taxonomy_columns = [f"Taxonomy Code {i}" for i in range(1, 16)]
df[taxonomy_columns] = df[taxonomy_columns].apply(lambda row: pd.Series([val for val in row if pd.notna(val)] + [None] * (15 - len([val for val in row if pd.notna(val)]))), axis=1,meta={0: 'object', 1: 'object', 2: 'object', 3: 'object', 4: 'object', 5: 'object', 6: 'object', 7: 'object', 8: 'object', 9: 'object', 10: 'object', 11: 'object', 12: 'object', 13: 'object', 14: 'object'})
df.compute()
print(f"Step 10: Shifted non-empty values in 'Taxonomy Code 1' to 'Taxonomy Code 15' columns upward")
...
df = df[column_order]
df.compute()
print(f"Step 11: Reordered DataFrame columns based on 'Column Order' sheet")

# Step 12:Order rows by "NPI" column
###########################################################################################
#df = df.sort_values(by="NPI")
#df.compute()
print(f"Step 12: Ordered DataFrame rows by 'NPI' column")

# Step 13: Write DataFrame to CSV file
###########################################################################################
df.compute()  # Ensure all computations are done before writing to CSV
df.to_csv("NPI Test Output_.csv",single_file=True, index=False)
